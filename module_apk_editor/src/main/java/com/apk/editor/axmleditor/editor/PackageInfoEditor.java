 ...
 * null)); //设置版本号、版本名和包名，不建议修改包名，会导致app 无法运行
...
public class PackageInfoEditor extends BaseEditor<PackageInfoEditor.EditorInfo> {
    ...
    protected void editor() {
        ...
        if (node != null) {
            ...
            int appPkgIndex = node.getAttrStringForKey(pkgName);
            String originPkgName = stringBlock.getStringFor(appPkgIndex);
            System.out.println("origin package name: " + originPkgName);
            ...
            if (editorInfo.verCodeHasEdit) {
                ...
                for (BTagNode.Attribute attr : attributes) {
                    if (attr.mName == verCode) {
                        attr.setValue(TypedValue.TYPE_INT_DEC, editorInfo.versionCode); //十进制int
                        // 值直接使用
                    }
                }
                stringBlock.setString(editorInfo.verCode_Value,
                        String.valueOf(editorInfo.versionCode));
            }
    ...
    public static class EditorInfo {
        ...
        public String toString() {
            return "EditorInfo{" + "versionCode=" + versionCode + ", versionName='" + versionName + '\'' + ", packageName='" + packageName + '\'' + ", verCode_Value=" + verCode_Value + ", verName_Value=" + verName_Value + ", pkgName_Value=" + pkgName_Value + ", verCodeHasEdit=" + verCodeHasEdit + ", verNameHasEdit=" + verNameHasEdit + ", pkgNameHasEdit=" + pkgNameHasEdit + '}';
        }
    }
}
